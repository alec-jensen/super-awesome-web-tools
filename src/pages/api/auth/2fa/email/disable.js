// @ts-nocheck
// API endpoint to disable email 2FA

import { getSessionUser } from '../../../../../lib/auth/session.js';
import { disableEmail2FA } from '../../../../../lib/auth/emailTwoFactor.js';
...
export async function POST({ request, cookies }) {
  ...
  try {
    ...
    // Check if user has other 2FA methods enabled (excluding email since we're disabling it)
    ...
    // Disable email 2FA
    await disableEmail2FA(sessionUser.id);
    ...
    return new Response(JSON.stringify({ ok: false, message: 'Failed to disable email 2FA' }), {
      status: 500,
      headers: { 'Content-Type': 'application/json' }
    });
  }
