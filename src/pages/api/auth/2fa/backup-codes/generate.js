// @ts-nocheck
// API endpoint to generate new backup codes
...
export async function POST({ request, cookies }) {
  ...
  try {
    ...
    if (hasAny2FA) {
      // Require 2FA verification to regenerate backup codes
      if (!twoFactorCode) {
    ...
    return new Response(JSON.stringify({ ok: false, message: 'Failed to generate backup codes' }), {
      status: 500,
      headers: { 'Content-Type': 'application/json' }
    });
  }
